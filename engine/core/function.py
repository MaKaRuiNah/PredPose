...
class CommonFunction(BaseFunction):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.PE_Name = kwargs.get("PE_Name", "PREDPOSE")
        ##
        self.max_iter_num = 0
    ...
    def train(self, model, epoch, optimizer, dataloader, tb_writer_dict, **kwargs):
        ...
        for iter_step in range(self.max_iter_num):
            # previous_input: bz*(3frame_len)*384*288,flow_input: bz*(2frame_len)*384*288
            # future_target: bz*(3future_frame_len)*384*288 heatmap: bz*(17frame_len)*96*72
            previous_input,flow_input,future_target, target_heatmaps_all, target_heatmaps_weight_all, meta = next(self.dataloader_iter)
            #self._before_train_iter(current_frame)
            ...
            #     outputs = model(current_frame.cuda())
            # elif self.PE_Name == "PREDPOSE":
            #concat_input = torch.cat((prev2, prev1,flow_frame), 1).cuda()

            outputs = model(previous_input,flow_input)
            # else:
            #     outputs = model(current_frame.cuda())
            ...
            losses.update(loss.item(), previous_input.size(0))
            ...
            if iter_step % self.cfg.PRINT_FREQ == 0 or iter_step >= self.max_iter_num - 1:
                ...
                msg = 'Epoch: [{0}][{1}/{2}]\t' \
                      ...
                      'Accuracy {acc.val:.3f} ({acc.avg:.3f})\t'.format(epoch, iter_step, self.max_iter_num, batch_time=batch_time,
                                                                        speed=previous_input.size(0) / batch_time.val,
    ...
    def train_pred1(self, model, epoch, optimizer, dataloader, tb_writer_dict, **kwargs):
        self.tb_writer = tb_writer_dict["writer"]
        self.global_steps = tb_writer_dict["global_steps"]
        logger = logging.getLogger(__name__)
        batch_time = AverageMeter()
        ...
        for iter_step in range(self.max_iter_num):
            ...
            if self.PE_Name == "HRNET":
                ...
                concat_input = torch.cat((input_sup_A, input_sup_B), 1).cuda()
    ...
    def train_pred2(self, model, epoch, optimizer, dataloader, tb_writer_dict, **kwargs):
        self.tb_writer = tb_writer_dict["writer"]
        self.global_steps = tb_writer_dict["global_steps"]
        logger = logging.getLogger(__name__)
        batch_time = AverageMeter()
        ...
        for iter_step in range(self.max_iter_num):
            input_x, target_heatmaps, target_heatmaps_weight, meta = next(self.dataloader_iter)
            self._before_train_iter(input_x)
    ...
    def eval(self, model, dataloader, tb_writer_dict, **kwargs):
        ...
        with torch.no_grad():
            ...
            for iter_step in range(self.max_iter_num):
                current_frame, prev2, prev1, flow_frame, target_heatmaps, target_heatmaps_weight, meta = next(self.dataloader_iter)
                if phase == VAL_PHASE:
                    self._before_val_iter(current_frame)
                ...
                #     if vis_dict:
                #         self._running_val_iter(vis_dict=vis_dict, model_input=[prev2, prev1, flow_frame])
                ...
                num_images = current_frame.size(0)
                ...
                image_path.extend(meta['img_path'])
                idx += num_images
        ...
        dataset.evaluate(self.cfg, all_preds, result_output_dir, all_boxes)
    ...
    def eval_pred1(self, model, dataloader, tb_writer_dict, **kwargs):
    ...
    def eval_pred2(self, model, dataloader, tb_writer_dict, **kwargs):
        ...
        with torch.no_grad():
            ...
            for iter_step in range(self.max_iter_num):
                ...
                self.global_steps += 1
                #
                #self._after_val_iter(meta["img_path"], preds, maxvals, vis_output_dir, center, scale)
