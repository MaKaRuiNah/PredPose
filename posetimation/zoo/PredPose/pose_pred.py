...
from posetimation.layers import BasicBlock, ChainOfBasicBlocks, DeformableCONV, EncodeImage
...
class PredPose_flow(BaseModel):

    def __init__(self, cfg, phase, **kwargs):
        super(PredPose_flow, self).__init__()
        ...
        self.encode_image = EncodeImage(self.previous_frame_len*3,self.previous_frame_len*3)
        self.encode_flow = EncodeImage(self.previous_frame_len*2,self.previous_frame_len*2)
    ...
    def forward(self, x,flow):
        ...
        if not x.is_cuda or not flow.is_cuda:
            x.cuda()
            flow.cuda()
        ...
        #current_rough_heatmaps, previous_rough_heatmaps, pred_rough_heatmaps = rough_heatmaps.split(true_batch_size, dim=0)
        estimate_heatmaps = torch.cat(rough_heatmaps.split(true_batch_size, dim=0),1) # bz * (3frame_len)*96*72
        down_img = self.encode_image(x)
        down_flow = self.encode_flow(flow)
        ...
        # pred_rough_heatmaps_list = pred_rough_heatmaps.split(1, dim=1)
        # temp_support_fuse_list = []
        # for joint_index in range(self.num_joints):
        #     temp_support_fuse_list.append(current_rough_heatmaps_list[joint_index])
        #     temp_support_fuse_list.append(previous_rough_heatmaps_list[joint_index])
        #     temp_support_fuse_list.append(pred_rough_heatmaps_list[joint_index])
        ...
        # for d_index, dilation in enumerate(self.deformable_conv_dilations):
        #     offsets = self.offsets_list[d_index](support_heatmaps)
        #     masks = self.masks_list[d_index](support_heatmaps)
    ...
    def get_net(cls, cfg, phase, **kwargs):
        model = PredPose_flow(cfg, phase, **kwargs)
        return model
