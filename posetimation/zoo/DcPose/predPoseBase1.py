...
class DcPose_Pred1(BaseModel):

    def __init__(self, cfg, phase, **kwargs):
        super(DcPose_Pred1, self).__init__()
    ...
    def forward(self, x):
        ...
        true_batch_size = int(rough_heatmaps.shape[0] / 2)

        # rough heatmaps in sequence frames
        pred1_rough_heatmaps, pred2_rough_heatmaps = rough_heatmaps.split(true_batch_size, dim=0)

        # Difference A and Difference B
        output_heatmaps = 2*pred1_rough_heatmaps - pred2_rough_heatmaps
    ...
    def get_net(cls, cfg, phase, **kwargs):
        model = DcPose_Pred1(cfg, phase, **kwargs)
        return model
